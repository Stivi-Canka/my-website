class UpdatesLoader {
  ...
  async loadUpdates() {
    ...
    try {
      const updateFiles = [
        "stanford-ms-mse-admission/metadata.json",
      ...
      const updatePromises = updateFiles.map((file) =>
        this.loadUpdateFile(file),
      );
  ...
  applyFilter() {
    if (this.showFeaturedOnly) {
      this.updates = this.allUpdates.filter(
        (update) => update.featured === true,
      );
    } else {
